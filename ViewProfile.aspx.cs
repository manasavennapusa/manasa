...
public partial class Admin_company_empmaster : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            //else MyImg.ImageUrl = "Upload/photo/image.jpg";
    ...
    protected void bind_PhotoId_detail(string empcode)
    {
        ...
        {
            connection = activity.OpenConnection();
            sqlstr = "select * from Emp_PhotoId_Doc where empcode = '" + empcode + "'";
    ...
    protected void bind_Address_proof(string empcode)
    {
        ...
        {
            connection = activity.OpenConnection();
            sqlstr = "select * from Emp_Address_Doc where empcode = '" + empcode + "'";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
            if (ds.Tables[0].Rows.Count < 1)
                return;
    ...
    protected void bind_Other_proof(string empcode)
    {
        ...
        {
            connection = activity.OpenConnection();
            sqlstr = "select * from Emp_Other_Doc where empcode = '" + empcode + "'";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
            if (ds.Tables[0].Rows.Count < 1)
                return;
...
COALESCE (tbl_intranet_employee_jobDetails.emp_fname, '')+ COALESCE (dbo.tbl_intranet_employee_jobDetails.emp_m_name, '') + COALESCE (dbo.tbl_intranet_employee_jobDetails.emp_l_name, '') AS Name
FROM EmpHistory 
...
where EmpHistory.Empcode='" + emp_code.Trim().ToString() + "'";
    ...
    protected void MyImg_Click(object sender, ImageClickEventArgs e)
    {
        trimg.Visible = true;
    ...
    protected void btnimg_Click(object sender, EventArgs e)
    {
        ...
        //{
        file_name = Session["empcode"] + System.DateTime.Now.GetHashCode().ToString() + Path.GetFileName(fileupload1.PostedFile.FileName);
        string targetPath = Server.MapPath("upload/photo/" + file_name);
        System.IO.Stream strm = fileupload1.PostedFile.InputStream;
        var targetFile = targetPath;
        //Based on scalefactor image size will vary
        GenerateThumbnails(0.5, strm, targetFile);
        //file_name = Session["empcode"] + System.DateTime.Now.GetHashCode().ToString();
        ...
        sqlparam[0] = new SqlParameter("@photo", SqlDbType.VarChar, 100);
        sqlparam[0].Value = ViewState["Photo"].ToString();

        sqlparam[1] = new SqlParameter("@empcode", SqlDbType.VarChar, 100);
        sqlparam[1].Value = Session["empcode"].ToString();

        sqlstr = "update tbl_intranet_employee_jobDetails set photo=@photo where empcode='" + Session["empcode"].ToString() + "'";
        int i = SQLServer.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString.ToString(), CommandType.Text, sqlstr, sqlparam);
        if (i < 0)
        {
            string str = "<script> alert('Employee Photo Not Uploaded Sucessfully')</script>";
            Page.RegisterStartupScript("Employee", str.ToString());
            MyImg.ImageUrl = "Upload/photo/" + Session["photo"].ToString();
        }
        else
        {
            string str = "<script> alert('Employee Photo Uploaded Sucessfully')</script>";
            Page.RegisterStartupScript("Employee", str.ToString());
            trimg.Visible = false;
            bind_job_detail(Session["empcode"].ToString());
            Session["photo"] = ViewState["Photo"].ToString();
            MyImg.ImageUrl = "Upload/photo/" + ViewState["Photo"].ToString();
        }
  ...
  where A.empcode = '" + emp_code + "' ) T";
    ...
    protected void bind_job_detail(string emp_code)
    {
        try
        {
            ...
            string sqlstr = @"SELECT tbl_intranet_employee_jobDetails.empcode, tbl_intranet_employee_jobDetails.photo,
tbl_intranet_employee_jobDetails.card_no, tbl_intranet_employee_jobDetails.emp_gender,tbl_intranet_employee_jobDetails.emp_fname, 
tbl_intranet_employee_jobDetails.emp_m_name, tbl_intranet_employee_jobDetails.emp_l_name,tbl_intranet_employee_status.employeestatus,
(CASE WHEN tbl_intranet_employee_jobDetails.emp_doj='01/01/1900' THEN '' ELSE CONVERT(CHAR(50),tbl_intranet_employee_jobDetails.emp_doj, 106) END) doj  , 
tbl_intranet_employee_jobDetails.ext_number,tbl_intranet_employee_jobDetails.Status, tbl_intranet_branch_detail.branch_name, 
tbl_intranet_grade.gradename, tbl_login.login_id,tbl_intranet_designation.designationname, tbl_intranet_division.division_name, 
tbl_intranet_role.role,tbl_internate_departmentdetails.department_name,(CASE WHEN tbl_intranet_employee_jobDetails.salary_cal_from='01/01/1900' THEN '' ELSE CONVERT(CHAR(50),tbl_intranet_employee_jobDetails.salary_cal_from, 106) END)salary_cal_from,
...
tbl_intranet_employee_jobDetails.probationenddate,(CASE WHEN tbl_intranet_employee_jobDetails.deputationstartdate='01/01/1900' THEN '' ELSE CONVERT(CHAR(50), tbl_intranet_employee_jobDetails.deputationstartdate, 106) END)deputationstartdate,
...
(CASE WHEN tbl_intranet_employee_jobDetails.confirmationdate='01/01/1900' THEN '' ELSE CONVERT(CHAR(50), tbl_intranet_employee_jobDetails.confirmationdate, 106) END)confirmationdate   
 ...
 left join tbl_internate_department_type on tbl_internate_department_type.dept_type_id=tbl_intranet_employee_jobDetails.dep_type_id
    ...
    protected void bind_personalinfo(string empcode)
    {
        try
        {
            ...
            sqlstr = @"SELECT empcode, f_fname, f_mname, f_lname, m_fname, m_mname, m_lname, bloodgrp, maritalstatus, religion,
                ...
                dlno, s_fname, s_mname, s_lname, (CASE WHEN s_dob = '01/01/1900' THEN '' ELSE CONVERT(CHAR(51), s_dob, 106) END) s_dob, s_gender, no_child, mobile_no, email_id,bankbranch,ifsc, (CASE WHEN passportissuedate = '01/01/1900' THEN '' ELSE CONVERT(CHAR(51), passportissuedate, 106) END) passportissuedate ,(CASE WHEN passportexpiraydate = '01/01/1900' THEN '' ELSE CONVERT(CHAR(51),passportexpiraydate, 106) END) passportexpiraydate ,
    ...
    protected void bind_Training_detail(string empcode)
    {
        try
        {
            connection = activity.OpenConnection(); sqlstr = "SELECT trainingname,personname,(CASE WHEN fromdate='01/01/1900' THEN '' ELSE CONVERT(CHAR(51), fromdate, 106) END)fromdate ,(CASE WHEN todate='01/01/1900' THEN '' ELSE CONVERT(CHAR(51), todate, 106) END) todate,remarks  FROM tbl_intranet_employee_trainingdetail where empcode = '" + empcode + "'";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
            if (ds.Tables[0].Rows.Count < 1)
                return;
