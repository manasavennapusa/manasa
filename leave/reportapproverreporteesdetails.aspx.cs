...
public partial class leave_reportapproverreporteesdetails : System.Web.UI.Page
{
    string sqlstr;
    ...
    public void bindemployee()
    {
        ...
        {
            connection = activity.OpenConnection();
            sqlstr = @"select distinct coalesce(tbl_intranet_employee_jobDetails.emp_fname,'') + ' ' + coalesce(tbl_intranet_employee_jobDetails.emp_m_name,'') + ' ' + coalesce(tbl_intranet_employee_jobDetails.emp_l_name,'') as name ,tbl_intranet_employee_jobDetails.empcode,tbl_intranet_employee_jobDetails.degination_id,tbl_intranet_designation.designationname,
                tbl_intranet_employee_jobDetails.branch_id,tbl_intranet_branch_detail.branch_name
                from tbl_intranet_employee_jobDetails 
				left join tbl_leave_employee_hierarchy eh on eh.approverid=tbl_intranet_employee_jobDetails.empcode
    ...
    protected void btn_search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                sqlstr = sqlstr + " AND emp_fname like '%" + txt_employee.Text.Replace("'", "''").Trim().ToString() + "%' or emp_m_name like '%" + txt_employee.Text.Replace("'", "''").Trim().ToString() + "%' or emp_l_name like '%" + txt_employee.Text.Replace("'", "''").Trim().ToString() + "%' or empcode like '%" + txt_employee.Text.Replace("'", "''").Trim().ToString() + "%' or emp_fname + ' ' + emp_m_name like '%" + txt_employee.Text.Replace("'", "''").Trim().ToString() + "%' or emp_fname + ' ' + emp_l_name like '%" + txt_employee.Text.Replace("'", "''").Trim().ToString() + "%' or emp_m_name + ' ' + emp_l_name like '%" + txt_employee.Text.Replace("'", "''").Trim().ToString() + "%' or emp_fname + ' ' + emp_m_name + ' ' + emp_l_name like '%" + txt_employee.Text.Replace("'", "''").Trim().ToString() + "%'";
            }
