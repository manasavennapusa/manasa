...
public partial class leave_Linemanager_update_leave : System.Web.UI.Page
{
    ...
    protected Boolean validateapplyleave()
    {
        ...
        try
        {
            ...
            {
                //Utility.DateFormat(txt_sdate.Text.ToString(CultureInfo.InvariantCulture)).ToString()
                //Utility.DateFormat(txt_edate.Text.ToString(CultureInfo.InvariantCulture)).ToString()
    ...
    protected int Insertapplyleave(int status, SqlConnection connection, SqlTransaction transaction)
    {
        ...
        if (divfull.Visible == true)
        {
            //Utility.DateFormat(txt_sdate.Text).ToString(CultureInfo.InvariantCulture)
           // Utility.DateFormat(txt_edate.Text).ToString(CultureInfo.InvariantCulture)
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            if (fromdate.Month != DateTime.Now.Month)
            {
                ...
                if (ds2.Tables[0].Rows.Count > 0)
                {
                    ...
                    string str4 = "UPDATE tbl_payroll_employee_attendence_detail SET mode='" + displayleavename + "', flag=1 WHERE empcode='" + empcode + "' AND date BETWEEN '" + Convert.ToDateTime(fromdate).ToString("MM/dd/yyyy") + "' AND '" + Convert.ToDateTime(todate).ToString("MM/dd/yyyy") + "'";
