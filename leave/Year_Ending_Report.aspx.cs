...
public partial class leave_Year_Ending_Report : System.Web.UI.Page
{
    string strsql;
    ...
    protected void btn_search_Click(object sender, EventArgs e)
    {
        ...
        string Role_Query = @" select ln.role from tbl_login ln inner join tbl_intranet_employee_jobDetails ej on ln.empcode = ej.empcode 
                                   where ln.empcode = '" + Session["empcode"].ToString() + "'";
        DataSet DS_Role = SQLServer.ExecuteDataset(connection, CommandType.Text, Role_Query);
        
        int Role = Convert.ToInt32(DS_Role.Tables[0].Rows[0]["role"].ToString());
        ...
        try
        {
            if(Role == 13)
            {
                strsql = @" select 
						...
						where EH.approverid = '" + Session["empcode"].ToString() + "' and UE.Calenderid= " + Ddl_Calender_Year.SelectedValue;
            }
            else if(Role == 1)
            {
                strsql = @" select 
            ...
            else
            {
                strsql = @" select 
                        ...
                        UE.OpeningBalance
                         from tbl_leave_Upload_year_Ending UE inner join tbl_leave_createleavepolicy Cp on
                        UE.policyid = Cp.policyid inner join tbl_leave_createleave Cl on UE.leaveid = Cl.leaveid 
                        where  UE.Calenderid= " + Ddl_Calender_Year.SelectedValue;
            }
