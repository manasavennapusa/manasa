...
using Common.Data;
public partial class leave_hrupdatecompofffforapproval : System.Web.UI.Page
{
    ...
    //        ds = DBTask.ExecuteDataset(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString.ToString(), CommandType.StoredProcedure, "[sp_leave_entitledcompoff]", sqlparm1);
    ...
    //        SqlConnection connection = activity.OpenConnection();
    //        ds = DBTask.ExecuteDataset(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString.ToString(), CommandType.StoredProcedure, "sp_leave_viewempdetail", sqlparm);
    ...
    protected void validateapplycompoff()
    {
        ...
        try
        {
            ...
            ds = DBTask.ExecuteDataset(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString.ToString(), CommandType.StoredProcedure, "[sp_leave_validatecompoff]", sqlparm);
    ...
    protected Boolean validate_applydate()
    {
        ...
        try
        {
            ...
            ds = DBTask.ExecuteDataset(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString.ToString(), CommandType.StoredProcedure, "sp_leave_validate_applied_date", sqlparam);
            if (ds.Tables[0].Rows.Count > 0)
            {
    ...
    protected void btn_submit_Click1(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            transaction.Commit();
            if (leaveid > 0)
                list.Add("Compoff leave applied for approval successfully.");
    ...
    protected void updateleaveapplication(int leavestatus, int status, int leaveid, SqlConnection connection, SqlTransaction transaction)
    {
        ...
        approverstatus = Convert.ToInt32(SQLServer.ExecuteNonQuery(connection, CommandType.Text, transaction, sqlstr, sqlparm));
