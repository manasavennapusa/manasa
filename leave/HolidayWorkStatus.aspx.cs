...
public partial class leave_HolidayWorkStatus : System.Web.UI.Page
{
    string _companyId;
    #region PageLoad
    ...
    protected string linkleave(string empcode, int id, int approvalstatus)
    {
        ...
        if (approvalstatus == 0)
            return "<a href='EditApplyOD.aspx?q=" + encoded + "' title='view detail' class='link05'>View</a>";
        else
    ...
    protected void bind()
    {
        try
        {
            ...
            DataSet ds = SQLServer.ExecuteDataset(connection, CommandType.StoredProcedure, "sp_leave_fetch_compoff_mark_userstatus", sqlparm);
