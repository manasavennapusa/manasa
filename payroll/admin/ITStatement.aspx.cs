...
public partial class payroll_admin_ITStatement : System.Web.UI.Page
{
  ...
  where  empcode = '" + empcode + "'";
    ...
    private void GenerateITStatement(string FYear, string Month, int MonthId, string EmpCode)
    {
        ...
        try
        {
            ...
            string Heading = "<p class='head'>Income tax statement for the financial year " + FYear + "<p>";
            ...
            Query = @"select distinct SD.PAYHEADID ,SD.PAYHEAD
                           ...
                           where S.YEAR='" + FYear + "' and S.EMPCODE = '" + EmpCode + "' and S.STATUS = 1 and H.payhead_type = 0 and SD.PAYHEADID not in (28,56) order by SD.PAYHEADID";
            ...
            foreach (DataRow Row in Ds.Tables[0].Rows)
            {
                ...
                for (int m = 0; m < PayMonth.Length; m++)
                {
                    ...
                    {
                        PayHeadValue = GetComponentAmount(Connection, Row["PAYHEADID"].ToString(), EmpCode, FYear, GetMonthName(PayMonth[m]));
            ...
            string TotalGross = "<p>Sub Total(A): " + GetTotalGrossPerYear(Connection, EmpCode, FYear, Month) + "</p>";
            string Perquisite = "<p>C) <b>Perquisites</b></p><p style='text-align:left;'>Sub Total(C): " + GetPerquisite(Connection, EmpCode, FYear, Month) + "</p>";
            ...
            Result.InnerHtml = Heading + SubHeading + EmpInfo + A + "<br/>" + TotalGross + "<br/>" + B + "<br/>" + Perquisite + "<br/>" + PreviousEmployement + "<br/>" + OtherIncome + "<br/>" + _GrossTotalIncome + "<br/>" + PT16 + "<br/>" + LessExemptionUnderSection + "<br/>" + Less6A + "<br/>" + _TotalDeduction + _TaxableAmount + _RoundToNearest10 + _Tax + _RebateUS87 + _Surcharge + _EducationCess + _TotalTax + _TotalTaxTillPaid + _MonthRemaining + _TDS;
    ...
    private string GetComponentAmount(SqlConnection Connection, string CId, string EmpCode, string FYear, string Month)
    {
        string Query = @"select SD.AMOUNT
                              from tbl_payroll_employee_salary S 
                              inner join tbl_payroll_employee_salarydetail SD on S.SALARYID = SD.SALARYID
                              inner join tbl_payroll_payhead H on SD.PAYHEADID = H.id
                              where S.YEAR='" + FYear + "' and S.EMPCODE = '" + EmpCode + "' and S.STATUS = 1   and S.MONTH = '" + Month + "' and SD.PAYHEADID = " + CId + "";
