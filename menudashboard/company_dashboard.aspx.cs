...
public partial class menudashboard_company_dashboard : System.Web.UI.Page
{
    string sqlstr;
...
where lm.status=1 and lm.empcode='" + usercode + "' and cl.displayleave='PL'";
    ...
    #region Binding of Location
    [WebMethod]
    public static List<object> GetChartData()
    {

        string query = @"SELECT bd.branch_name,bd.branch_id FROM tbl_intranet_branch_detail bd
inner join tbl_intranet_companydetails cd on bd.company_id=cd.companyid
order by bd.company_id";
    ...
    #region Binding of Department

    [WebMethod]
    public static List<object> LocWiseDeptChart()
    {

        string query = @"select department_name,departmentid from tbl_internate_departmentdetails ";
            ...
            using (SqlCommand cmd = new SqlCommand(query))
            {
                ...
                using (SqlDataReader sdr = cmd.ExecuteReader())
                {
                    while (sdr.Read())
                    {
                        chartData.Add(new object[]
                    {
                        sdr["department_name"],sdr["departmentid"]
                    });
                    }
                }
                con.Close();
    ...
    #region Binding of Designation
    [WebMethod]
    public static List<object> DesignationChart()
    {
        string query = @"select id,designationname from tbl_intranet_designation";
        string constr = ConfigurationManager.ConnectionStrings["connectionString"].ConnectionString;
        List<object> chartData = new List<object>();
        chartData.Add(new object[]
    {
    ...
    #region Binding of Locationwise Head Count
    [WebMethod]
    public static List<object> CompanyWiseHeadCountChart()
    {

        string query = @"select cd.companyname,COUNT(*)[TotalEmployee] 
            ...
            using (SqlCommand cmd = new SqlCommand(query))
            {
                ...
                using (SqlDataReader sdr = cmd.ExecuteReader())
                {
                    while (sdr.Read())
                    {
                        chartData.Add(new object[]
                    {
                        sdr["companyname"],sdr["TotalEmployee"]
                    });
                    }
                }
                con.Close();
