...
public partial class Travel_PendingTravelForm : System.Web.UI.Page
{
    string sqlstr;
    ...
    private void ValidateCancelButton(int travelid)
    {
        ...
        try
        {
            ...
            ds = SQLServer.ExecuteDataset(Connection, CommandType.StoredProcedure, "sp_travel_validatecancel", parm);
            if (ds.Tables[0].Rows.Count > 0)
            {
                if (ds.Tables[0].Rows[0]["status"].ToString() == "NA")
                    btnCancelTravel.Visible = false;
                else btnCancelTravel.Visible = true;
            }
        }
        catch (Exception ex)
        {
