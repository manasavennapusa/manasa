...
public partial class Travel_AddExpanseDetails : System.Web.UI.Page
{
    decimal grdtotal = 0;
    string sqlstr;
    DataSet ds;
    ...
    protected void Insert_lodging_details()
    {
        ...
        if (Request.QueryString["travelid"] != null)
        {
            ...
            sqlParam[2] = new SqlParameter("@lodgingstartdate", SqlDbType.DateTime);
            sqlParam[2].Value = txtStartdate.Text;

            sqlParam[3] = new SqlParameter("@lodgingenddate", SqlDbType.DateTime);
            sqlParam[3].Value = txtEnddate.Text;

            sqlParam[4] = new SqlParameter("@noofdays", SqlDbType.Decimal);
            sqlParam[4].Value = txtNoofdays.Text == "" ? "0" : txtNoofdays.Text;
    ...
    protected void Insert_Miscellaneous_details()
    {
        ...
        if (Request.QueryString["travelid"] != null)
        {
            ...
            sqlParam[5] = new SqlParameter("@exptype", SqlDbType.VarChar, 20);
    ...
    protected void Insert_Personalcar_details()
    {
        ...
        if (Request.QueryString["travelid"] != null)
        {
            ...
            sqlParam[7] = new SqlParameter("@exptype", SqlDbType.VarChar, 20);
    ...
    protected void btngeneralsubmit_Click(object sender, EventArgs e)
    {
        ...
        if (flag > 0)
        {
            SendEmail(ID, 0);
            SendEmail(ID, 1);
            Response.Redirect("ExpanseDetails.aspx?submit=true");
        }
        else
        {
    ...
    void SendEmail(int travelId, int type)
    {
        if (type == 0)
        {
            EmailFactory email = new Smart.HR.Common.Mail.Module.Travel.UponSubmissionOfExpenseRequest();
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    if (row["official_email_id"].ToString() != "")
                    {
                        try
                        {
                            EmailFactory email = new Smart.HR.Common.Mail.Module.Travel.UponSubmissionOfExpenseRequestApprover();
                            EmailClient client = new EmailClient(email);
    ...
    {
        Response.Redirect("ExpanseDetails.aspx");
    }
