...
public partial class appraisal_ViewBH_kra_golas : System.Web.UI.Page
{
    private string _companyId, _userCode, EmployeeCode;
    ...
    protected void Binddetails()
    {
        ...
        try
        {
            ...
            //            string str = @"select apt.assessment_id as initializeid,aps.asd_id,aps.flow_id,aps.assessment_id,apt.empcode,
            //aps.kca,aps.weightage,aps.kpi,aps.role,aps.emp_comments,aps.mng_comments,aps.desigid  
            //from tbl_appraisal_assessment_details aps 
            //inner join tbl_appraisal_assessment apt on aps.desigid=apt.desigid
            //where apt.status in(1,3) and (apt.R_cycle=2 or R_cycle=5 or R_cycle=3) and
            //apt.appcycle_id= '" + Convert.ToInt32(Session["appcycle"]) + "' and  apt.empcode='" + EmployeeCode + "' ";

            string str = @"select apt.empcode, role_name_of_the_goal,kca_kra_desired_outcome_impact,kpi_milestone_to_check_improvement,
weightage_timeline_and_support_required,
...
where (apt.G1_cycle=1 or apt.G2_cycle=1) and (apt.R_cycle=2 or apt.R_cycle=3 or apt.R_cycle=1 or apt.R_cycle=0 or apt.R_cycle=4) and
goal_cycle1.applycyclid= '" + Convert.ToInt32(Session["appcycle"]) + "' and apt.empcode='" + EmployeeCode + "' order by apt.empcode";
