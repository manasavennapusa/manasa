...
public partial class appraisal_ApproverIncreamentDetails : System.Web.UI.Page
{
    decimal grdTotal = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["approve"] != null)
            Common.Console.Output.Show("Application has been Approved Sucessfully.");
        if (Request.QueryString["reject"] != null)
            Common.Console.Output.Show("Application has been Rejected Sucessfully.");
    }
    ...
    private void bindHREmpIncreamentDetails(string EmployeeCode)
    {
        ...
        try
        {
            ...
            if (dse.Tables[0].Rows.Count >= 1)
            {
                ...
                lblcurbonus.Text = dse.Tables[0].Rows[0]["curbouns"].ToString();
                lblcurctc.Text = dse.Tables[0].Rows[0]["curctc"].ToString();
                lblincramount.Text = dse.Tables[0].Rows[0]["curincreasedamount"].ToString();
                //lblrevdate.Text = dse.Tables[0].Rows[0]["wefdate"].ToString();---------------Changed by Irshad-------------------------------------
    ...
    private void bindgrid()
    {
        ...
        try
        {
            ...
            DataSet ds = SQLServer.ExecuteDataset(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString.ToString(), CommandType.StoredProcedure, "[sp_appraisal_listofIncreatement_approver]", sqlparam);
    ...
    private void InsertIncrementStatus(SqlConnection Connection, SqlTransaction _transaction, int status)
    {
        ...
        if (status == 5)
        {
            if (txtcomm.Text != "")
            {
                txtcomm.Text = "<h6><b><font color=red>Comments</font> added by " + Session["empcode"] + "-" + Session["name"] + "(" + Session["rolename"] + ") on " + DateTime.Now +
        ...
        else
        {
            if (txtcomm.Text != "")
            {
                txtcomm.Text = "<h6><b><font color=green>Comments</font> added by " + Session["empcode"] + "-" + Session["name"] + "(" + Session["rolename"] + ") on " + DateTime.Now +
                                   " :</b><br>" + txtcomm.Text + "</h6>";
            }
            else txtcomm.Text = "";
        }
        ...
        SQLServer.ExecuteDataset(Connection, CommandType.Text, _transaction, str2);
    ...
    protected void btnapprove_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("ApproverIncreamentDetails.aspx?approve=true");
    }
    protected void btnreject_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("ApproverIncreamentDetails.aspx?reject=true");
    }
}