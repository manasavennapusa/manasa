...
public partial class appraisal_FreezeGoalByHR_cycle2 : System.Web.UI.Page
{
    decimal grdTotal = 0;
    decimal grdTotal2 = 0;
    string EmployeeCode = "";
...
//            string sql = @"select app.appcycle_id,isnull(emp_fname ,'''')+ ''+isnull( emp_m_name,'''')+  ''+isnull( emp_l_name,'''') as name,
//rtrim(empjob.empcode) as empcode,empdesg.designationname as designation,empdept.department_name as dept,app.status,
//app.create_by,app.freeze,app.behavior_color
...
//where 1=1 and empjob.emp_status in(1,3) 
//and (app.G1_cycle=1 and app.G2_cycle=1) 
//and app.status in(1,3) and app.create_by='" + UserCode + "' and freeze=1 and R_cycle=4 order by empjob.empcode";
            string sql = @"select app.appcycle_id,isnull(emp_fname ,'''')+ ''+isnull( emp_m_name,'''')+  ''+isnull( emp_l_name,'''') as name,
rtrim(empjob.empcode) as empcode,empdesg.designationname as designation,empdept.department_name as dept,app.status,
app.create_by,app.freeze,app.behavior_color,lmrtng2.BH_appraiser_cmnt
from tbl_intranet_employee_jobDetails empjob
inner join tbl_intranet_designation empdesg on empjob.degination_id=empdesg.id
inner join tbl_internate_departmentdetails empdept on empjob.dept_id=empdept.departmentid
inner join tbl_appraisal_assessment app on app.empcode= empjob.empcode 
inner join tbl_appraisal_approver_LM_rating_details_for_cycle_2 lmrtng2 on lmrtng2.empcode= empjob.empcode 
where 1=1 and empjob.emp_status in(1,3) and (app.G1_cycle=1 and app.G2_cycle=1) 
and app.status in(1,3) and app.create_by='" + UserCode + "' and freeze in (1) and lmrtng2.BH_appraiser_cmnt is not null order by empjob.empcode";
            DataSet ds = SQLServer.ExecuteDataset(Connection, CommandType.Text, sql);
...
where apt.status=1 and apt.appcycle_id='" + Convert.ToInt32(Session["appcycle"]) + "' and  apt.empcode='" + empcode + "'";
    ...
    protected void btnFreeze_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            //{
                ...
                if (Request.QueryString["empcode"] != null)
                {
                    ...
                    string sqlstr = @"update tbl_appraisal_assessment set freeze=2,freezecycle2date =getdate() where status=1 and appcycle_id=" + Convert.ToInt32(Session["appcycle"]) + " and empcode='" + empcode + "'";
                    //DataSet ds1 = SQLServer.ExecuteDataset(Connection, CommandType.Text, sqlstr);
                    //bindgrid();
                ...
                if (Request.QueryString["NA_empcode"] != null)
                {
                    ...
                    string sqlstr = @"update tbl_appraisal_assessment set freeze=2,G1_cycle=4,freezecycle2date =getdate() where status=1 and appcycle_id=" + Convert.ToInt32(Session["appcycle"]) + " and empcode='" + empcode + "'";
                    //DataSet ds1 = SQLServer.ExecuteDataset(Connection, CommandType.Text, sqlstr);
                    //bindgrid();
                    i = DBTask.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString.ToString(), CommandType.Text, sqlstr);
                }
        ...
        finally
        {
            ...
            if (i > 0)
            {
                Response.Redirect("FreezeGoalByHR_cycle2.aspx?updated=true");
            }
        }
    }
        ...
        try
        {
            ...
            foreach (GridViewRow row in gveligible.Rows)
            {
                ...
                if (chkselect.Checked)
                {
                    ...
                    string sqlstr = @"update tbl_appraisal_assessment set freeze=2,freezecycle2date=getdate() where empcode='" + emp.Text + "'";
                    i = DBTask.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString.ToString(), CommandType.Text, sqlstr);
                    if (i > 0)
                    {
                        Response.Redirect("FreezeGoalByHR_cycle2.aspx?freze=true");
                    }
    ...
    {
        Response.Redirect("FreezeGoalByHR_cycle2.aspx");
    }
