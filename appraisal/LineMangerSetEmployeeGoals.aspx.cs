...
public partial class appraisal_LineMangerSetEmployeeGoals : System.Web.UI.Page
{
    decimal grdTotal = 0;
    decimal grdTotal2 = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
           ...
           // bindempgrid2();
            ...
            {
                Output.Show("Goal Initiated Successfully");
            }
                ...
                if (gveligible.Rows.Count > 0)
                {
                    foreach (GridViewRow row in gveligible.Rows)
                    {
                        ...
                        if (chk.Checked)
                        {
                            ...
                            Output.AssignParameter(sqlparam, 3, "@desigid", "Int", 0,"");
                            ...
                            if (cnt == 0)
                            {
                                ...
                                //ds_id = int.Parse(sqlparam[4].Value.ToString());
                                if (ds_id.ToString() != "")
                                {
                                    ...
                                    SQLServer.ExecuteScalar(Connection, CommandType.Text, str3);
                                    //-------------insert workflow-------------//
                                    ...
                                    Output.AssignParameter(sqlparam1, 9, "@applycyclid", "Int", 0, Session["appcycle"].ToString());
                            ...
                            if (Session["goal_details"] != null)
                            {
                                ...
                                for (int i = 0; i < dtable.Rows.Count; i++)
                                {
                                    ...
                                    SqlParameter[] sqlparam2 = new SqlParameter[13];
                                    ...
                                    Output.AssignParameter(sqlparam2, 9, "@desigid", "Int", 0, "");
                                    ...
                                    Output.AssignParameter(sqlparam2, 12, "@applycyclid", "Int", 0, Session["appcycle"].ToString());
                                    ...
                                    foreach (GridViewRow row1 in gvGoals.Rows)
                                    {
                                        ...
                                        SqlParameter[] sqlparam2 = new SqlParameter[10];
                                        ...
                                        Output.AssignParameter(sqlparam2, 6, "@designationid", "String", 10,"");
                                        Output.AssignParameter(sqlparam2, 7, "@designationname", "String", 100, "");
                                        Output.AssignParameter(sqlparam2, 8, "@createdby", "String", 60, empcode);
                                        Output.AssignParameter(sqlparam2, 9, "@applycyclid", "Int", 0, Session["appcycle"].ToString());
...
//                                    DataSet ds = SQLServer.ExecuteDataset(Connection, CommandType.Text, sqlstr);
                ...
                Response.Redirect("LineMangerSetEmployeeGoals.aspx?sub=true");
                return;
            }
            else
            {
    ...
    protected void bindempgridIndirect()
    {
        try
        {
            string sqlstr = @" select empjob.empcode + ' - ' + empjob.emp_fname as name,empjob.empcode as empcode
 from tbl_intranet_employee_jobDetails empjob
 inner join tbl_employee_approvers app on app.empcode=empjob.empcode
where app.app_reportingmanager='" + empcode.Trim() + "' and empjob.emp_status in (1,3)";
...
//    }
    ...
    private void slectedDeptGrid()
    {
        ...
        {
            connection = activity.OpenConnection();
            string sqlstr = "select asd_id, role, kpi, weightage, kca, desigid from dbo.tbl_appraisal_assessment_details where  create_by ='" + empcode + "'";
    ...
    protected void gvGoals_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        dtable = (DataTable)Session["goal_details"];
        dtable.Rows[a]["role_name_of_the_goal"] = textrole.Text;
        dtable.Rows[a]["kca_kra_desired_outcome_impact"] = textkca.Text;
        dtable.Rows[a]["kpi_milestone_to_check_improvement"] = textkpi.Text;
        dtable.Rows[a]["weightage_timeline_and_support_required"] = textweight.Text;
...
set role='" + textrole.Text + "',kpi='" + textkpi.Text + "',weightage='" + textweight.Text + "',kca='" + textkca.Text + "' where asd_id = '" + abc.ToString().Trim() + "'";
    ...
    {
        Session["goal_details"] = dtable;
        //ds = DBTask.ExecuteDataset(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString, "getchildmenu", Convert.ToInt32(dtable));
        if (dtable.Rows.Count > 0)
        {
            gvGoals.DataSource = dtable;
            gvGoals.DataBind();
    ...
    protected void Gridindirect_PreRender(object sender, EventArgs e)
    {
        if (Gridindirect.Rows.Count > 0)
        {
            Gridindirect.HeaderRow.TableSection = TableRowSection.TableHeader;
        }
    }
    protected void ddlempcode_DataBound(object sender, EventArgs e)
    {
    ...
    {
        string empcode = ddlempcode.SelectedValue.ToString();

         try
        {
            string sqlstr = @"select distinct empappr.appcycle_id,isnull(emp_fname ,'''')+''+isnull( emp_m_name,'''')+ ''+isnull( emp_l_name,'''') as name,
...
left join tbl_appraisal_flow app_flow on empappr.empcode=app_flow.empcode
 inner join tbl_employee_approvers app on app.empcode=empappr.empcode
where app.app_reportingmanager='" + ddlempcode.SelectedValue.ToString() + "'and empappr.status in (0,1) and empappr.appcycle_id='"+ Convert.ToInt32(Session["appcycle"]) + "' and empjob.emp_status in (1,3)  and cycle.status=1 ";
            ...
            if (ds.Tables.Count > 0)
            {
                Gridindirect.DataSource = ds;
                Gridindirect.DataBind();
            }
        }
        catch (Exception ex)
        {
...
inner join tbl_appraisal_eligible_employee empappr on empjob.empcode=empappr.empcode
inner join tbl_appraisal_cycle cycle on cycle.appcycle_id=empappr.appcycle_id
and cycle.quater=empappr.quater and cycle.APP_year=empappr.APP_year
left join tbl_appraisal_flow app_flow on empappr.empcode=app_flow.empcode and app_flow.applycyclid=cycle.appcycle_id
 inner join tbl_employee_approvers app on app.empcode=empappr.empcode
where app.app_reportingmanager='" + empcode.Trim() + "' and empappr.status=0 and empappr.appcycle_id='" + Dropappcycle_id.SelectedValue.ToString() + "' and empjob.emp_status in (1,3)  and empjob.empcode not in(select empcode from tbl_appraisal_flow where cycle_type='" + Dropappcycle_id.SelectedValue.ToString() + "' ) order by empjob.empcode";
