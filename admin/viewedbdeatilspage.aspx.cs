...
public partial class admin_viewedbdeatilspage : System.Web.UI.Page
{
    public int i;
...
tbl_intranet_employee_jobDetails.emp_l_name,tbl_intranet_employee_status.employeestatus,(CASE WHEN tbl_intranet_employee_jobDetails.emp_doj='01/01/1900' THEN '' ELSE CONVERT(CHAR(50),tbl_intranet_employee_jobDetails.emp_doj, 106) END) doj  , 
tbl_intranet_employee_jobDetails.ext_number,tbl_intranet_employee_jobDetails.Status, tbl_intranet_branch_detail.branch_name,tbl_intranet_grade.gradename, 
tbl_login.login_id,tbl_intranet_designation.designationname, tbl_intranet_division.division_name, tbl_intranet_role.role,tbl_internate_departmentdetails.department_name,
...
tbl_intranet_employee_jobDetails.reason_leaving,tbl_intranet_employee_jobDetails.salutation ,tbl_intranet_employee_jobDetails.official_email_id,
tbl_intranet_employee_jobDetails.official_mob_no,tbl_internate_employee_subtype.emp_subtype_name,tbl_internate_employee_type.emp_type_name,tbl_internate_department_type.dept_type_name,
...
WHERE empcode = '" + empcode + "'";
    ...
    protected void bind_contactdetails(string empcode)
    {
        ...
        {
            connection = activity.OpenConnection();
            sqlstr = @"SELECT pre_add1, pre_Add2, pre_city, pre_state, pre_country, pre_zip, pre_phone, per_add1, per_add2, per_city,
per_state, per_country, per_zip, per_phone, empcode,mode,modeoftransport,emergency_contact_no,emergency_name,emergency_relation,
...
(CASE WHEN dob = '01/01/1900' THEN '' ELSE CONVERT(CHAR(50), dob, 106) END) dob,(CASE WHEN doa = '01/01/1900' THEN '' ELSE CONVERT(CHAR(50), doa, 106) END) doa, 
dlno, s_fname, s_mname, s_lname, (CASE WHEN s_dob = '01/01/1900' THEN '' ELSE CONVERT(CHAR(51), s_dob, 106) END) s_dob, s_gender, no_child, 
mobile_no, email_id,bankbranch,ifsc, (CASE WHEN passportissuedate = '01/01/1900' THEN '' ELSE CONVERT(CHAR(51), passportissuedate, 106) END) passportissuedate,
...
(case when paymentmode=0 then 'Bank' else case when paymentmode='1' then 'Cheque' else 'Cash' end end) paymentmode ,driving_lic_no,dribing_lic_iss_date,driving_lic_exp_date,landlineno
FROM tbl__employee_personal_details p
 ...
 where empcode = '" + empcode + "'";
    ...
    protected void bind_child(string empcode)
    {
        ...
        {
            connection = activity.OpenConnection();
            string sqlstr = "select id,child_name ,(CASE WHEN childdob='01/01/1900' THEN '' ELSE CONVERT(CHAR(51), childdob, 106) END)child_dob,gender from tbl_employee_childrendetail where empcode ='" + empcode + "'";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
            //if (ds.Tables[0].Rows.Count < 1)
            //    return;
...
FROM tbl_employee_edcational  where empcode = '" + empcode + "'";
    ...
    protected void bind_Professional_Qualification(string empcode)
    {
        try
        {
            connection = activity.OpenConnection();
            sqlstr = @"SELECT id,empcode,education,school,percentage,yearfrom as from_year,yearto as to_year,specialization  
            FROM tbl_employee_professional where empcode = '" + empcode + "'";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
    ...
    protected void bind_Training_detail(string empcode)
    {
        ...
        {
            connection = activity.OpenConnection(); 
            sqlstr = @"SELECT trainingname,personname,(CASE WHEN fromdate='01/01/1900' THEN '' ELSE CONVERT(CHAR(51), fromdate, 106) END)fromdate ,
(CASE WHEN todate='01/01/1900' THEN '' ELSE CONVERT(CHAR(51), todate, 106) END) todate,remarks  FROM tbl_intranet_employee_training where empcode = '" + empcode + "'";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
            if (ds.Tables[0].Rows.Count < 1)
                return;
...
from tbl_intranet_employee_jobDetails where empcode='" + Empcode + "'";
    ...
    //    {
    //        activity.CloseConnection();
    //        Output.Show("Employee Details Submitted sucessfully!!!");
