...
public partial class Admin_company_empmaster : System.Web.UI.Page
{
    ...
    private void BindEmpHistory(string emp_code)
    {
        ...
        {
           string empqu =@"SELECT  j.EmpCode, dbo.tbl_intranet_designation.designationname, 
                              d.department_name, r.role, 
                              COALESCE (j.emp_fname, '')+ 
                              COALESCE (j.emp_m_name, '') + 
                              COALESCE (j.emp_l_name, '') AS Name
                              FROM tbl_intranet_employee_jobDetails j INNER JOIN 
                              tbl_login logs on logs.empcode=j.empcode inner join 
                              dbo.tbl_intranet_role r ON logs.role = r.id
                              INNER JOIN dbo.tbl_intranet_designation ON j.degination_id = dbo.tbl_intranet_designation.id
                              INNER JOIN dbo.tbl_internate_departmentdetails d ON d.departmentid = j.dept_id 
                              where j.empcode='" + emp_code.Trim().ToString() + "'";
           DataSet ds4 = SQLServer.ExecuteDataset(connection, CommandType.Text, empqu);

           EmpHistGrid.DataSource = ds4;
 ...
 left join tbl_intranet_employee_jobDetails HRCB on A.app_hr_cb = HRCB.empcode
  ...
  left join tbl_internate_department_type on tbl_internate_department_type.dept_type_id=tbl_intranet_employee_jobDetails.dep_type_id
    ...
    protected void bind_payrolldetails(string empcode)
    {
        ...
        {
            connection = activity.OpenConnection();
            string sqlstr = "SELECT empcode,esi_no,esi_disp,pf_no,pf_no_dept,pan_no,ward,ptno,uan FROM tbl_intranet_employee_payrollDetails  WHERE empcode = '" + empcode + "'";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
            if (ds.Tables[0].Rows.Count < 1)
                return;
    ...
    protected void bind_child(string empcode)
    {
        ...
        {
            connection = activity.OpenConnection();
            string sqlstr = "select id,child_name ,(CASE WHEN childdob='01/01/1900' THEN '' ELSE CONVERT(CHAR(11), childdob, 106) END)child_dob,gender from tbl_intranet_employee_childrendetail where empcode ='" + empcode + "'";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
            //if (ds.Tables[0].Rows.Count < 1)
            //    return;
