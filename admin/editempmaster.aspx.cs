...
public partial class Admin_company_empmaster : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["empcode"] != null)
        {
            ...
            if (!IsPostBack)
            {
                ...
                //Binding Data Of Employee HIstory Starts Here
                BindEmpHistory(emp_code);

                //Binding Employee History Ends Here
    ...
    private void BindEmpHistory(string emp_code)
    {
        ...
        string EmpQry = @"Select distinct EmpHistory.EmpCode, tbl_intranet_designation.designationname,tbl_internate_departmentdetails.department_name,tbl_intranet_role.role, 
EmpHistory.UpdatedBy, COALESCE (tbl_intranet_employee_jobDetails.emp_fname, '')+ COALESCE (dbo.tbl_intranet_employee_jobDetails.emp_m_name, '') + COALESCE (dbo.tbl_intranet_employee_jobDetails.emp_l_name, '') AS Name
FROM EmpHistory 
INNER JOIN tbl_intranet_role ON EmpHistory.EmpRole = tbl_intranet_role.id 
INNER JOIN tbl_intranet_designation ON EmpHistory.DesignationId = tbl_intranet_designation.id 
INNER JOIN tbl_internate_departmentdetails ON EmpHistory.DepartmentId = tbl_internate_departmentdetails.departmentid 
INNER JOIN tbl_intranet_employee_jobDetails ON EmpHistory.EmpCode = tbl_intranet_employee_jobDetails.empcode
where EmpHistory.Empcode='" + emp_code.Trim().ToString() + "'";
 ...
 left join tbl_intranet_employee_jobDetails HRCB on A.app_hr_cb = HRCB.empcode
  ...
  where A.empcode = '" + emp_code + "' ) T";
            ...
            if (ds.Tables[0].Rows.Count > 0 )
            {
                ...
                if (ds.Tables[0].Rows[0]["app_reportingmanager"].ToString() != "")
                {
                ...
                if (ds.Tables[0].Rows[0]["app_dotted_linemanager"].ToString() != "")
                {
                ...
                if (ds.Tables[0].Rows[0]["app_businesshead"].ToString() != "")
                {
                ...
                if (ds.Tables[0].Rows[0]["app_finance"].ToString() != "")
                {
                ...
                if (ds.Tables[0].Rows[0]["app_admin"].ToString() != "")
                {
                ...
                if (ds.Tables[0].Rows[0]["app_hr"].ToString() != "")
                {
                ...
                if (ds.Tables[0].Rows[0]["app_hrd"].ToString() != "")
                {
                ...
                if (ds.Tables[0].Rows[0]["app_management"].ToString() != "")
                {
                ...
                if (ds.Tables[0].Rows[0]["app_hr_cb"].ToString() != "")
                {
                ...
                if (ds.Tables[0].Rows[0]["clr_networkdept"].ToString() != "")
                {
    ...
    protected void bind_job_detail(string emp_code)
    {
        ...
        {
            connection = activity.OpenConnection();
            string sqlstr = @"SELECT tbl_login_1.empcode, emp.uid,emp.card_no, emp.emp_gender, emp.emp_fname,emp.emp_m_name, emp.emp_l_name,
emp.emp_status, emp.dept_id, emp.division_id,emp.degination_id, emp.Grade, emp.branch_id,(CASE WHEN emp.emp_doj='01/01/1900' THEN '' ELSE CONVERT(CHAR(50), emp.emp_doj, 106) END)emp_doj, 
emp.ext_number, emp.photo,emp.Status, tbl_login_1.login_id,tbl_login_1.role,(CASE WHEN emp.salary_cal_from='01/01/1900' THEN '' ELSE CONVERT(CHAR(50), emp.salary_cal_from, 106) END)salary_cal_from,
(CASE WHEN emp.emp_doleaving='01/01/1900' THEN '' ELSE CONVERT(CHAR(50), emp.emp_doleaving, 106) END)emp_doleaving,emp.reason_leaving,
emp.salutation ,emp.employee_type,emp.sub_emp_type,emp.official_email_id,emp.official_mob_no,emp.broadgroupid,emp.entityid,emp.supervisorcode,
emp.hodcode,emp.corporatereportingcode,emp.probationperiod,emp.probationenddate,emp.deputationstartdate,emp.deputationenddate,emp.gradetype,
emp.noticeperiod,emp.confirmationdate,emp.employee_type,emp.dep_type_id  FROM tbl_login AS tbl_login_1 
INNER JOIN tbl_intranet_employee_jobDetails emp ON tbl_login_1.empcode = emp.empcode where emp.empcode = '" + emp_code + "'";
            ...
            if (drpempstatus.SelectedValue.Trim() == "1")
            {
                ViewState["empstatus"] = "1";
               
            }
            else
            {
                ViewState["empstatus"] = "2";
    ...
    //private void bind_employeesubtype(int emp_type_code)
    //{
    //    try
    //    {
    //        connection = activity.OpenConnection();
    ...
    //        sqlstr = "select emp_subtype_id,emp_subtype_name from tbl_internate_employee_subtype where emp_subtype_id=" + emp_type_code + " order by emp_subtype_name";
    //        ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
    //        if (ds.Tables[0].Rows.Count > 0)
    //        {
    ...
    protected void bind_personalinfo(string empcode)
    {
        ...
        {
            connection = activity.OpenConnection();
            sqlstr = @"SELECT empcode, f_fname, f_mname, f_lname, m_fname, m_mname, m_lname, bloodgrp, maritalstatus, religion,bankbranch,
            ifsc,passportissuedate,passportexpiraydate,(CASE WHEN dob = '01/01/1900' THEN '' ELSE CONVERT(CHAR(50), dob, 106) END) dob, 
(CASE WHEN doa = '01/01/1900' THEN '' ELSE CONVERT(CHAR(50), doa, 106) END) doa, dlno, s_fname, s_mname, s_lname,
(CASE WHEN s_dob = '01/01/1900' THEN '' ELSE CONVERT(CHAR(50), s_dob, 106) END) s_dob, s_gender, no_child, mobile_no, email_id,bank_name,
ac_number,passport_number,paymentmode,bank_name_reimbursement,ac_number_reimbursement,driving_lic_no,dribing_lic_iss_date,driving_lic_exp_date,
landlineno FROM tbl_intranet_employee_personalDetails where empcode = '" + empcode + "'";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
    ...
    protected void btngeneralsubmit_Click(object sender, EventArgs e)
    {
        ...
        string str = "<script> alert('Employee Personal Detail Save sucessfully!!!')</script>";
    ...
    protected void edit_probation_to_confirmation(string emp_code, SqlConnection connection, SqlTransaction transaction)
    {
        ...
        sqlparam[1]=new SqlParameter("@emp_statusconf", SqlDbType.Int);
        sqlparam[1].Value = drpempstatus.SelectedValue;

        sqlparam[2] = new SqlParameter("@emp_probation", SqlDbType.VarChar, 10);
        sqlparam[2].Value = ViewState["empstatus"].ToString();

        sqlparam[3] = new SqlParameter("@confirmation", SqlDbType.DateTime);
        if (txt_confirmationdate.Text != "")
        {
            if (drpempstatus.SelectedItem.Text != "Confirmed")
            {
            ...
            {
                sqlparam[3].Value = txt_confirmationdate.Text;
            }
        }
        else
        {
          ...
          sqlparam[4]=new SqlParameter("@emp_gender", SqlDbType.VarChar,10);
          sqlparam[4].Value = drpgender.SelectedItem.ToString();

          sqlparam[5] = new SqlParameter("@Creaetdby", SqlDbType.VarChar, 50);
          sqlparam[5].Value = Session["empcode"].ToString();




          ds = SQLServer.ExecuteDataset(connection, CommandType.StoredProcedure, transaction, "[sp_leave_prorata_leave_confomation]", sqlparam);
    ...
    void check_equal(DataTable dtable)
    {
        if (dtable.Rows[0][4] != null)
        {
            string secondyearedu = drp_edu_qualification.SelectedItem.Text.Trim();
    ...
    private void Getdefaultlable()
    {
        ...
        if (dt.Rows.Count >= 15)
        {
            ...
            Label3.Text = ds.Tables[0].Rows[16]["labelname"].ToString();
        }
    ...
    protected void bind_emp_doc(string empcode)
    {
        ...
        try
        {
            sqlstr = "SELECT empcode,default1,default2,default3,default4,default5,default6,default7,default8,default9,default10,default11,default12,default13,default14,default15 FROM tbl_intranet_employee_docupload where empcode = '" + empcode + "'";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
            if (ds.Tables[0].Rows.Count > 0)
            {
              ...
              "' target='_blank'>" + ds.Tables[0].Rows[0]["default14"].ToString() + "</a>" : "No exisitng file found";
                Label5.Text = (ds.Tables[0].Rows[0]["default15"].ToString() != "") ? "<a href='../upload/employeedocuments/" + ds.Tables[0].Rows[0]["default15"].ToString() +
    ...
    protected void btnjob_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            else if (txtdol.Text != "" && doj.Text != "")
            {
                ...
                if ((drpempstatus.SelectedValue == "3") && (ViewState["empstatus"].ToString() == "1"))
                {
                    edit_probation_to_confirmation(txtempcode.Text, connection, transaction);
                }

                string _Desig_Hist, _Deprt_Hist, _Role_Hist;
                string st2 = "select DesignationId,DepartmentId,EmpRole from EmpHistory where EmpCode='" + emp_code + "'";
                ...
                if (ds1.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (drpdegination.SelectedValue != _Desig_Hist || drpdepartment.SelectedValue != _Deprt_Hist || drprole.SelectedValue != _Role_Hist)
                    {
                        string st = "insert into EmpHistory(EmpCode,DesignationId,DepartmentId,EmpRole,UpdatedBy,DateAdded,status)values('" + txtempcode.Text.Trim() + "','" + drpdegination.SelectedValue.Trim().ToString() + "','" + drpdepartment.SelectedValue.Trim().ToString() + "','" + drprole.SelectedValue.Trim().ToString() + "','" + Session["empcode"].ToString() + "','" + System.DateTime.Today.ToShortDateString() + "',0)";
                ...
                if (Session["Inserted_Emp_code"] != null)
                {
                    ...
                    // resetjobdetails();
                    //lbl_msg.Text = "Employee Detail has sucessfully Inserted";
                    string str = "<script> alert('Employee Job Details Saved Successfully')</script>";
                    ScriptManager.RegisterStartupScript(dd, dd.GetType(), "xx", str, false);
    ...
    protected void btncontact_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Session["Inserted_Emp_code"] != null)
            {
                ...
                //lbl_msg.Text = "Employee Detail has sucessfully Inserted";Employee Approvers Details Saved Successfully!!!
    ...
    private void BindApproverDetails(string empcode, SqlConnection connection, SqlTransaction transaction)
    {
        ...
        Output.AssignParameter(sqlparm1, 10, "@clr_accountsdept", "String", 50, txtaccdeptclr.Text.Split('>')[0].Trim());
    ...
    protected void btnapprover_Click(object sender, EventArgs e)
    {
        ...
        string str = "<script> alert('Employee Approvers Detail Saved Successfully!!!')</script>";
        // Common.Console.Output.Show("'Employee Code already exists, Please change Employee Code.");
    ...
    protected void btn_save_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            newparameter[0].Value = TextBox1.Text;

            newparameter[1] = new SqlParameter("@individual", SqlDbType.Int);
            newparameter[1].Value = TextBox2.Text;
            ...
            newparameter[3].Value = TextBox4.Text;

            newparameter[4] = new SqlParameter("@createdby", SqlDbType.VarChar, 50);
            newparameter[4].Value = _userCode.ToString();


            ds = DBTask.ExecuteDataset(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString.ToString(), CommandType.StoredProcedure, "sp_uphealth_insurance", newparameter);
            // return true;
        }
        catch (Exception ex)
        {
        ...
        {
            //activity.CloseConnection();
            Output.Show("Health insurance is updated sucessfully");
        }
    }

    protected void Bindhealth(String emp_code)
    {

        try
        {
            connection = activity.OpenConnection();
            string sqlstr = @"select benefits,individual,familyplanfour,familyplansix from tbl_health_insurance where createdby='" + emp_code.ToString() + "' ";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
            if (ds.Tables[0].Rows.Count < 1)
                return;
            TextBox1.Text = ds.Tables[0].Rows[0]["benefits"].ToString();
            //string s = ds.Tables[0].Rows[0]["individual"].ToString();
            TextBox2.Text = ds.Tables[0].Rows[0]["individual"].ToString(); ;
            TextBox3.Text = ds.Tables[0].Rows[0]["familyplanfour"].ToString();
            TextBox4.Text = ds.Tables[0].Rows[0]["familyplansix"].ToString();
            //lbldoc.Text = ds.Tables[0].Rows[0]["file_path"].ToString();     
        }
        catch (Exception ex)
        {
            Output.Log("During validation: " + ex.Message + ".    " + DateTime.Now);
    ...
    protected void ddl_familysix_SelectedIndexChanged(object sender, EventArgs e)
    {

    }

    protected void ddl_familyfour_SelectedIndexChanged(object sender, EventArgs e)
    {

    }

    protected void ddl_individual_SelectedIndexChanged(object sender, EventArgs e)
    {
    ...
    private void bind_employeesubtype()
    {
        try
        {
            connection = activity.OpenConnection();
            //sqlstr = "select emp_type_code,emp_subtype_name from tbl_internate_employee_subtype where emp_type_code=" + emp_type_code + " order by emp_subtype_name";
            sqlstr = "select emp_subtype_id,emp_subtype_name from tbl_internate_employee_subtype";
            ds = SQLServer.ExecuteDataset(connection, CommandType.Text, sqlstr);
            if (ds.Tables[0].Rows.Count > 0)
            {
                ddl_semp_type.DataSource = ds;
    ...
    protected void ddlpersonalstatus_SelectedIndexChanged1(object sender, EventArgs e)
    {
        if ((ddlpersonalstatus.SelectedValue.ToString() == "UNMARRIED") || (ddlpersonalstatus.SelectedValue == "0"))
        {
            tbl1.Visible = false;
