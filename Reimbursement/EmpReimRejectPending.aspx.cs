...
public partial class Reimbursement_EmpReimRejectPending : System.Web.UI.Page
{
    ...
    private void BindPendingDetailsAll()
    {
        ...
        try
        {
            ...
            else { Output.Show("No Pending Reimbursement Data Found"); }
        }
        catch (Exception ex)
        {
    ...
    private void BindDetailsAll()
    {
        ...
        try
        {
            ...
            Output.AssignParameter(parm, 3, "@flag", "Int", 0, "1");
            Connection = DA.OpenConnection();
            ds = SQLServer.ExecuteDataset(Connection, CommandType.StoredProcedure, "sp_Rb_get_empreimdetails_reject", parm);
            if (ds.Tables[0].Rows.Count > 0)
            {
                grdreim.DataSource = ds;
                grdreim.DataBind();
            }
            else { Output.Show("No Rejected Reimbursement Data Found"); }
        }
        catch (Exception ex)
        {
            Common.Console.Output.Log("During validation: " + ex.Message + ".    " + DateTime.Now);
    ...
    private void BindPendingDetails()
    {
        ...
        try
        {
            ...
            {
                grdpending.DataSource = "";
                grdpending.DataBind();
                Output.Show("No Pending Data Found");
            }
        }
        catch (Exception ex)
        {
    ...
    private void BindDetails()
    {
        ...
        try
        {
            ...
            else { Output.Show("No Rejectd Data Found"); }
        }
        catch (Exception ex)
        {
            Common.Console.Output.Log("During validation: " + ex.Message + ".    " + DateTime.Now);
