...
public partial class admin_edittemplogin : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ddl_semp_type.Items.Insert(0, new ListItem("---Select---", "0"));
            Bind_IdType();
            txtextccode.Text = "+91";
            bindHRLoginEmail();
    ...
    private bool ValidateTempEmployee()
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    div.Visible = false;
                    SmartHr.Common.Alert("Your are already submited details to HR.");
                    return false;
                }
                if ((ds.Tables[0].Rows[0]["status"].ToString() == "2")||(ds.Tables[0].Rows[0]["status"].ToString() == "1"))
                {
    ...
    protected void btngeneralsubmit_Click(object sender, EventArgs e)
    {
           ...
           // edit_Training_details(emp_code, connection, transaction);
            ...
            string appmsg = @"<html xmlns='http://www.w3.org/1999/xhtml'>" +
                 ...
                 "</head>" +
                  ...
                  "<tr>" +
                 "<td><asp:Label ID='lblname' runat='server' style='font-family:Georgia;font-weight:600'>Dear " + txt_created_by.Text.ToString() + " ,</asp:Label></td>" +
                 "</tr>" +
                  "<tr><td><br/></td></tr>" +
                 "<tr>" +
                "<td>" +
                  "<asp:Label ID='lblmessage' runat='server'>You have a pending Employee Details by <l style='font-style:Georgia'>" + txtfirstname.Text.ToString().Trim() + "</l>-<l style='font-style:Georgia'>" + txtempcode.Text.ToString().Trim() + " to be validated</l>.</asp:Label>" +
                  "</td>" +
                  "</tr>" +
                  "<tr><td><br/></td></tr>" +
                  "<tr>" +
            ...
            if (txt_hr_mail.Text != "")
            {
                sendmail_Template(txt_hr_mail.Text.ToString().Trim(), appmsg);
            }
        ...
        string str = "<script> alert('Submitted Successfully')</script>";
    ...
    protected void edit_Job_detail(string emp_code, SqlConnection connection, SqlTransaction transaction)
    {
        ...
        {
            sqlparam[18].Value = System.Data.SqlTypes.SqlString.Null;
        }
        else
        {
        ...
        if (txt_officialemail.Text == "0")
        {
            sqlparam[28].Value = System.Data.SqlTypes.SqlString.Null;
        }
        else
        {
        ...
        ds = SQLServer.ExecuteDataset(connection, CommandType.StoredProcedure, transaction, "sp_intranet_edit_jobdetails_for_temp", sqlparam);
    ...
    protected void btn_exp_add_Click(object sender, EventArgs e)
    {
        if (Convert.ToInt32(txt_exp_from.Text) <= Convert.ToInt32(DateTime.Now.Year) && Convert.ToInt32(txt_exp_to.Text) <= Convert.ToInt32(DateTime.Now.Year))
        {
            ...
            if (toYearFstPg <= fromYear)
            {
                Common.Console.Output.Show("Experience year should be greater than Educational Qual");
                return;
            }
            Ins_exp();
            txtcomp1.Text = "";
    ...
    {
        ddl_bank_name.Items.Insert(0, new ListItem("---Select---", "0"));
    }
    ...
    {
        ddl_bank_name_reimbursement.Items.Insert(0, new ListItem("---Select---", "0"));
    }
    private void Getdefaultlable()
    {
        ...
        lbl_adhar_card.Text = "Adhar Card";
    }
    ...
    protected void edit_emp_documents(string empcode, SqlConnection connection, SqlTransaction transaction)
    {
        ...
        //        adharcard = file_name + "_" + strFileName;//Server.MapPath("~//upload//employeedocuments//" + strFileName);
        //        sqlParam[16].Value = adharcard;
    ...
    protected void btnpersonal_Click(object sender, EventArgs e)
    {
        ...
        if (txt_DOB.Text != "")
        {
            //if (Convert.ToDateTime(txt_DOB.Text.ToString()) > Convert.ToDateTime(DateTime.Now.ToString()))
            //{
            //    Output.Show("Date of birth should be less than or equal to current date");
            //    return;
        ...
        //if (txt_passportissueddate.Text != "" && txt_passportexpdate.Text!="")
        //{
        //    if (Convert.ToDateTime(txt_passportissueddate.Text) > Convert.ToDateTime(txt_passportexpdate.Text))
        //    {
        //        Output.Show("Passport Issued Date should be less than or equal to Passport Expiry Date");
        //        return;
        //    }
        //}
        //if (txt_dr_iss_date.Text != "" && txt_dr_exp_date.Text != "")
        //{
        //    if (Convert.ToDateTime(txt_dr_iss_date.Text) > Convert.ToDateTime(txt_dr_exp_date.Text))
        //    {
        //        Output.Show("Driving Licence Issued Date should be less than or equal to Driving Licence Expiry Date");
        //        return;
        //    }
        //}
        try
    ...
    private void BindApproverDetails(string empcode, SqlConnection connection, SqlTransaction transaction)
    {
        ...
        //Output.AssignParameter(sqlparm1, 15, "@app_dotted_linemanager", "String", 50, "");
    ...
    //        activity.CloseConnection();
    //    }
    //    string str = "<script> alert('Employee Approvers Detail has been successfully Saved.')</script>";
    //    // Common.Console.Output.Show("'Employee Code already exists, Please change Employee Code.");
    ...
    protected void Insert_Emp_submitted_status(string emp_code, SqlConnection connection, SqlTransaction transaction)
    {
     ...
     //   SQLServer.ExecuteNonQuery(connection, CommandType.Text, transaction, sqlstr1);
                ...
                string sqlstr = @"update tbl_onboarding_templogin set submitstatus='S' where empcode ='" + emp_code + "'";
                SQLServer.ExecuteNonQuery(connection, CommandType.Text, transaction, sqlstr);
    ...
    protected void bindHRLoginEmail()
    {
        string sqlstr = @"select temp.createdby,temp.empcode,jd.empcode,jd.emp_fname,jd.official_email_id 
from tbl_onboarding_templogin temp
inner join tbl_intranet_employee_jobDetails jd on temp.createdby=jd.empcode 
where temp.empcode='" + UserCode + "'";
        DataSet ds = new DataSet();
        ds = DBTask.ExecuteDataset(ConfigurationManager.ConnectionStrings["intranetConnectionString"].ConnectionString.ToString(), CommandType.Text, sqlstr);
        txt_hr_mail.Text = ds.Tables[0].Rows[0]["official_email_id"].ToString();
        txt_created_by.Text = ds.Tables[0].Rows[0]["createdby"].ToString();
    }
    ...
    public bool sendmail_Template(string recievermailid, string bdy)
    {
        ...
        try
        {
            ...
            string subject = "Employee Information";
